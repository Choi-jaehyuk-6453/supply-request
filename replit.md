...
## Recent Changes
- 2026-01-21: 신청내역조회 품목 표시 및 월별 집계 자동 계산 수정
  - 피복 신청 시 품목명이 DB에 정상 저장되도록 수정
  - 신청내역조회에서 품목 컬럼 정상 표시
  - 신청서 생성 시 월별 집계 자동 업데이트 (관리자 모드에서 등록된 현장만)
  - 현장이 월별 집계에 미등록 시 경고 메시지 표시
- 2026-01-21: 신청내역 삭제 기능 추가
  - 신청내역조회에서 삭제 버튼(🗑️) 추가
  - 삭제 시 월별 집계 자동 차감 (단일 트랜잭션으로 데이터 일관성 보장)
- 2026-01-21: 폰트 문제 해결
